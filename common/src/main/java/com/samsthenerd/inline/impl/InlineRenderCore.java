...
public class InlineRenderCore {
    ...
    public static boolean textDrawerAcceptHandler(int index, Style style, int codepoint, RenderArgs args) {
        ...
        if(needsGlowChildren){
            Vector4f straightVec = new Vector4f(16, 16, 0, 0);
            Matrix4f mvpMatrix = RenderSystem.getModelViewMatrix().mul(RenderSystem.getProjectionMatrix(), new Matrix4f());
            straightVec.mul(mvpMatrix.mul(args.matrix(), new Matrix4f()));
            float len = straightVec.lengthSquared();
            int resScale = 8;
            ...
            glowStack.push();
            glowStack.translate(0, 4 * resScale, -50);
...
//                nativeImage.writeTo(MinecraftClient.getInstance().runDirectory.toPath().resolve("boop.png"));
                NativeImage frontImage = nativeImage.applyToCopy(getGlowyFunc(usableColor));
                nativeImage.apply(original -> ColorHelper.Argb.getAlpha(original) << 24 | 0x00_FFFFFF);
                // debug thingy

                MinecraftClient.getInstance().getFramebuffer().beginWrite(true);
                Identifier backTexId = MinecraftClient.getInstance().getTextureManager().registerDynamicTexture(new Identifier(Inline.MOD_ID, "glowtextureback").toTranslationKey(), new NativeImageBackedTexture(nativeImage));
                Identifier frontTexId = MinecraftClient.getInstance().getTextureManager().registerDynamicTexture(new Identifier(Inline.MOD_ID, "glowtexturefront").toTranslationKey(), new NativeImageBackedTexture(frontImage));
                TextureSprite backSprite = new TextureSprite(backTexId);
                TextureSprite frontSprite = new TextureSprite(frontTexId);
...
//                            SpritelikeRenderers.getRenderer(frontSprite).drawSpriteWithLight(frontSprite, drawContext, 0, -4, 0, 16, 16, trContext.light(), 0xFF_FFFFFF);
                            ...
                            matrices.translate(j * outlineScaleBack, k * outlineScaleBack, 0); // TODO: this scales with size too, prob unideal.
//                            SpritelikeRenderers.getRenderer(backSprite).drawSpriteWithLight(backSprite, drawContext, 0, -4, 0, 16, 16, trContext.light(), (glowColor & 0x00_FFFFFF) | (usableColor & 0xFF_000000));
                            SpritelikeRenderers.getRenderer(backSprite).drawSpriteWithLight(backSprite, drawContext, 0, -4, 0, 16, 16, trContext.light(), brighterGlow);
                        }
                        matrices.pop();
                    }
                }
                MinecraftClient.getInstance().getTextureManager().destroyTexture(backTexId);
                MinecraftClient.getInstance().getTextureManager().destroyTexture(frontTexId);
            } catch (Exception e){
